...
const loaded = async () => {
  await find(() => q('.shelf-row:not(.skeleton)') || q('.shelf-block .case-plaque') || q('.shelf-none'))
...
Object.assign(SCENES, {
  ...
  'sheet-movie':     { shot: 'view', run: async () => { await dismissGreeting(); await click('.shelf .shelf-main', firstOf('movie').title); await find(() => q('.sheet-overview')); await sleep(600) } },
  'sheet-album':     { shot: 'view', run: async () => { await dismissGreeting(); await shelfFilter('albums'); await click('.shelf .shelf-main', firstOf('album').title); await find(() => q('.sheet .hint, .sheet .tracks')) } },
  ...
  'focus-paper':     { shot: 'view', run: async () => { await dismissGreeting(); const a = await find(() => q('.shelf .shelf-play')); a.focus({ focusVisible: true }); scrollToBlock('the shelf') } },
  ...
  'shelf':           { shot: 'view', run: async () => { await dismissGreeting(); scrollToBlock('the shelf'); await sleep(300) } },
  'shelf-search':    { shot: 'view', run: async () => { await dismissGreeting(); await type('.shelf-search input', 'nas'); await sleep(200); scrollToBlock('the shelf'); await sleep(300) } },
  'shelf-albums':    { shot: 'view', run: async () => { await dismissGreeting(); await shelfFilter('albums'); scrollToBlock('the shelf'); await sleep(500) } },
  'shelf-empty-filter': { shot: 'view', run: async () => { await dismissGreeting(); await shelfFilter('shows'); await find(() => q('.shelf-none')); scrollToBlock('the shelf'); await sleep(300) } },
  'shelf-more':      { shot: 'full', run: async () => { await dismissGreeting(); await click('.shelf-actions .btn', 'more'); await find(() => document.querySelectorAll('.shelf-row').length > 20 || null); await sleep(400) } },
  // scroll to a named day heading (?day=sep 17)
  'shelf-day':       { shot: 'view', run: async () => {
    await dismissGreeting()
    const want = P.get('day')
    const more = () => byText('.shelf-actions .btn', 'more')
    let head = null
    while (!(head = byText('.shelf-day-head', want)) && more()) { more().click(); await sleep(250) }
    if (!head) throw new Error(`no day heading "${want}"`)
    window.__note = `heading reads "${head.querySelector('span').textContent}"`
    head.scrollIntoView({ block: 'center' }); await sleep(400)
  } },
  // time zone fixture (late=1): which day does a 9:30 pm eastern arrival land under? it must be the device's local day
  'shelf-late':      { shot: 'view', run: async () => {
    await dismissGreeting()
    const title = window.__harness.lateTitle
    const row = await find(() => [...document.querySelectorAll('.shelf .shelf-row')].find((r) => r.querySelector('.shelf-title')?.textContent === title))
    const day = row.closest('.shelf-day')
    const label = day.querySelector('.shelf-day-head span').textContent
    const local = new Date('2026-09-22T01:30:00Z')
    const expected = local.toLocaleDateString(undefined, { weekday: 'long' }).toLowerCase()
    const tz = Intl.DateTimeFormat().resolvedOptions().timeZone
    if (label !== expected && !(tz === 'UTC' && label === 'yesterday')) throw new Error(`"${title}" landed under "${label}", expected "${expected}" in ${tz}`)
    window.__note = `"${title}", added 9:30 pm eastern on monday sep 21 (01:30 utc tuesday): lands under "${label}" with the device in ${tz}`
    day.scrollIntoView({ block: 'center' }); await sleep(400)
  } },
  'shelf-loading':   { shot: 'view', run: async () => { await find(() => q('.shelf-row.skeleton')); scrollToBlock('the shelf'); await sleep(300) } },
  'shelf-failed':    { shot: 'view', run: async () => { await find(() => q('.shelf-block .case-plaque.error')); scrollToBlock('the shelf'); await sleep(300) } },
  'shelf-empty':     { shot: 'view', run: async () => { await find(() => q('.shelf-block .case-plaque.empty')); scrollToBlock('the shelf'); await sleep(300) } },
  ...
  'shelf-nomatch':   { shot: 'view', run: async () => { await dismissGreeting(); await type('.shelf-search input', 'qxzv plorb'); await find(() => q('.shelf-none')); scrollToBlock('the shelf'); await sleep(300) } },
  ...
  'loading':         { shot: 'full', run: async () => { await find(() => q('.shelf-row.skeleton')) } },
  'empty':           { shot: 'full', run: async () => { await find(() => byText('.case-plaque p', 'nothing on the servers')) } },
...
async function shelfFilter(label) { await click('.shelf-tab', label); await sleep(150) }
